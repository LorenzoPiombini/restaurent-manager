...
unsigned char clock_out(char* employee_id)
{
	...
	int j = 0, fnd = 0, pos[keys_ar->length];
	for(j = 0; j < keys_ar->length; j++) {
		if(keys_ar->types[j] == STR) {
			if(strstr((char*)keys_ar->k[j],employee_id) == NULL) {
				pos[j] = -1;
				continue;
			}else{	
				fnd++;
	...
	for(j = 0; j < keys_ar->length; j++) {
		...
		if(return_first_char((char*)keys_ar->k[pos[j]]) == 'o') {
			fnd--;
			continue;
		} else {
			break;
	...
	if(!__update("timecard",fd_tci,fd_tcd,data_to_add,keys_ar->k[pos[j]],keys_ar->types[pos[j]])) {
	...
	char* key_n = strdup((char*)keys_ar->k[pos[j]]);
	if(!key_n) {
	...
	for(j = 0; j < *pi; j++) {
		Node* node = NULL;
		if((node = delete(keys_ar->k[pos[j_old]],&ht[j],keys_ar->types[pos[j_old]])) == NULL)
			continue;	
		
		if(off_v == 0)
			off_v = node->value;

		if(node->key_type == STR)
			free(node->key.s);
		
		free(node);

		if(!set(key_n,STR,off_v,&ht[j])) {
...
unsigned char list_clocked_in_employee(char*** employee_list, int* size)
{
	...
	if(!keys_ar){
		fprintf(stderr,"retrive keys form index file filed %s:%d,\n",F,L-2);
		close_file(1,fd_tci);
		return 0;
	}
	...
	{
		register char c = return_first_char((char*)keys_ar->k[i]);
		if( c != 'm' ) {	
			free(keys_ar->k[i]);
			keys_ar->k[i] = NULL;
			continue;
	...
	*employee_list = calloc(sz,sizeof(char*));
	...
	for(i = 0; i < keys_ar->length; i++) {
		...
		char* key_fnd = NULL;
		if(!extract_employee_key((char*)keys_ar->k[i],&key_fnd,TIMECARD)) {
		...
		if(!(*employee_list)[j]){
			fprintf(stderr,"strdup() fialed, %s:%d.\n",F,L-2);
			free_keys_data(keys_ar);
			return 0;
		}
		free(key_fnd);
